...
my $sm_kokoonpano_url = "http://www.sm-liiga.fi/ottelut/kokoonpano";
...
while (<PELIT>) {
    ...
    if ((/0\t(.*?)\s*-\s*(.*?),\s*(\d+)\s*$/ && $param_liiga eq "sm_liiga") || (/0\t(.*?)\s*-\s*(.*?)\s*$/ && $param_liiga eq "sm_liiga") || (/\s+(\D.*?)\s*-\s*(.*?)\s*$/ && $param_liiga eq "nhl")) {
        $kotipelit{$1}++;
        $vieraspelit{$2}++;
        $kaikkipelit{$1}++;
        $kaikkipelit{$2}++;
...
sub update_menus {
    ...
    print "var _gaq = _gaq || [];\n";
    print "_gaq.push(['_setAccount', 'UA-37908082-1']);\n";
    ...
    print "<li><A HREF=\"$script_name?sub=uutiset\">Uutiset</A></li>\n";
    ...
    if ($param_sub =~ /uutiset/)             {print_uutiset()};
    if ($param_sub =~ /optimi_joukkue/)      {print_optimi_joukkue()};
    if ($param_sub =~ /kokoonpano/)          {print_kokoonpano()};
...
sub print_kokoonpano {
    ...
    my $data = $ua->get("$sm_kokoonpano_url/$param_game_nro.html");
 
    if ($data->is_success) {
        #print $data->decoded_content;  # or whatever
    } else {
	...
	if (/\"\d+\s+(.*?),\s+(.*?)\"/) {
	    ...
	    if ($koti_vieras == 2 && !defined $vieras) { $vieras = $pelaaja{$nimi}{'joukkue'}; }
	    if ($koti_vieras == 1 && !defined $koti) { $koti = $pelaaja{$nimi}{'joukkue'}; }
    ...
    print hidden(-name => 'sub', -default => "kokoonpano");
    print hidden(-name => 'game_nro', -default => $param_game_nro);
    print submit('Update');
    print endform;
    ...
    print "<tr>\n";
    for (my $i = 0; $i <= 1; $i++) {
        print "<th><center>Nimi</center></th>\n";
        print "<th><center>Pe</center></th>\n";
        print "<th><center>Ma</center></th>\n";
        print "<th><center>Sy</center></th>\n";
        print "<th><center>Pi</center></th>\n";
        print "<th><center>La</center></th>\n";
        print "<th><center>Arvo</center></th>\n";
        print "<th><center>LPP</center></th>\n";
        print "<th><center>Ennuste</center></th>\n";
        print "<th><center>LPP ennuste</center></th>\n";
    }
    print "</tr>\n";
	...
	for ($pelaaja_nro = 1; $pelaaja_nro <= 5; $pelaaja_nro++) {
	    print "<tr>\n";
	        ...
	        if (defined $kokoonpanot{$koti_vieras}{$kentta}{$pelaaja_nro}) {
	            ...
	            print "<td class=\"$td\">$pelaaja{$nimi}{'lpp'}<\/td>\n";
	            print "<td class=\"$td\">$pelaaja{$nimi}{'ennuste_pisteet'}<\/td>\n";
   		    ...
   		    print "<\/td>\n";
#	        } else {
#		    for (my $i = 1; $i <= 10; $i++) { print "<td class=\"$td\">&#32;<\/td>\n"; }
	        }
	    }
    ...
    print "<th colspan=\"20\"><center>Ei kokoonpanossa</center></th>\n";
...
sub print_uutiset {
    if (!-e "uutiset.htm") { return; }
    my $data = `cat uutiset.htm`;
